...
function PricingSlider({ value, onChange }: { value: number; onChange: (value: number) => void }) {
  ...
  <div className="relative w-full mb-8 md:mb-10 select-none">
      ...
      <div
        ref={trackRef}
  className="relative h-2 rounded-full bg-[hsl(224,65%,95%)] dark:bg-[hsl(224,65%,15%)] cursor-pointer"
        onClick={(e) => setFromClientX((e as React.MouseEvent).clientX)}
      >
        {/* Fill */}
        <div
          className="h-2 rounded-full bg-[hsl(174,77%,80%)] dark:bg-[hsl(174,86%,45%)] transition-[width] duration-200"
          style={{ width: `${percent}%` }}
        />

        {/* Thumb */}
        <div
          className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-10 h-10 rounded-full grid place-items-center shadow-[0_15px_30px_rgba(0,255,231,0.6)] dark:shadow-[0_15px_30px_rgba(0,255,231,0.3)] bg-[hsl(174,86%,45%)] hover:bg-[hsl(174,86%,40%)] active:scale-95 transition-all"
          style={{ left: `${percent}%` }}
          role="slider"
          aria-valuemin={0}
          aria-valuemax={4}
  ...
  return (
    <button
      ...
      className={`relative w-11 h-5 rounded-full transition-colors duration-200 focus:outline-none ${
        isYearly 
          ? "bg-[hsl(174,86%,45%)]" 
          : "bg-[hsl(223,50%,87%)] dark:bg-[hsl(223,50%,30%)]"
      }`}
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div
        className="absolute inset-x-0 top-0 bg-[url('/images/bg-pattern.svg')] dark:opacity-40 bg-no-repeat bg-[position:10%_0] bg-[length:280%_auto] h-[250px] md:bg-[position:center_0] md:bg-[length:100%_auto] md:h-[31.2vw] md:min-h-[220px]"
        aria-hidden
      />
  ...
  <div className="relative z-10 flex flex-col items-center pt-16 md:pt-24 pb-20 px-5 md:px-6">
        ...
        <div className="relative text-center mb-12 md:mb-16 max-w-md">
          ...
          <h1 
            className="text-[26px] md:text-[28px] lg:text-[32px] font-extrabold mb-2 md:mb-3 leading-tight text-[hsl(227,35%,25%)] dark:text-white"
          >
            Simple, traffic-based pricing
          </h1>
          <div 
            className="text-[13.5px] md:text-[15px] leading-relaxed text-[hsl(225,20%,60%)] dark:text-[hsl(225,20%,80%)]"
          >
        ...
        {/* Pricing Card */}
  <div className="bg-white dark:bg-[hsl(232,19%,20%)] rounded-[14px] shadow-[0_30px_60px_rgba(0,0,0,0.08)] dark:shadow-[0_30px_60px_rgba(0,0,0,0.2)] w-full max-w-[560px] md:max-w-[700px] overflow-hidden">
          ...
          <div className="px-6 md:px-10 lg:px-12 pt-8 md:pt-10 pb-8 md:pb-10">
            ...
            <div className="block md:hidden">
              ...
              <div className="text-center mb-10">
                <span className="text-xs font-extrabold tracking-[0.2em] uppercase text-[hsl(225,20%,60%)] dark:text-[hsl(225,20%,80%)]">
              ...
              {/* Price */}
              <div className="text-center mb-10">
                <span className="text-[44px] leading-none font-extrabold text-[hsl(227,35%,25%)] dark:text-white">
                  ${displayPrice}
                </span>
                <span className="text-sm ml-2 text-[hsl(225,20%,60%)] dark:text-[hsl(225,20%,80%)]">
                  / {isYearly ? "year" : "month"}
                </span>
              </div>
            </div>
            ...
            <div className="hidden md:block">
              <div className="flex items-center justify-between mb-12 lg:mb-16">
                <span className="text-sm font-extrabold tracking-[0.2em] uppercase text-[hsl(225,20%,60%)] dark:text-[hsl(225,20%,80%)]">
                  {currentPricing.pageviews} Pageviews
                </span>
                <div className="text-right">
                  <span className="text-4xl lg:text-5xl font-extrabold text-[hsl(227,35%,25%)] dark:text-white">
                    ${displayPrice}
                  </span>
                  <span className="text-sm ml-2 text-[hsl(225,20%,60%)] dark:text-[hsl(225,20%,80%)]">
                    / {isYearly ? "year" : "month"}
            ...
            <div className="flex items-center justify-center md:justify-end gap-3 text-xs md:text-sm">
              <span className="text-[hsl(225,20%,60%)] dark:text-[hsl(225,20%,80%)]">Monthly Billing</span>
              <BillingToggle isYearly={isYearly} onChange={() => setIsYearly(!isYearly)} />
              <span className="text-[hsl(225,20%,60%)] dark:text-[hsl(225,20%,80%)]">Yearly Billing</span>
              <span className="px-2 py-1 rounded-full text-xs font-extrabold ml-2 bg-[hsl(14,92%,95%)] dark:bg-[hsl(15,100%,15%)] text-[hsl(15,100%,70%)]">
                -25%
              </span>
            </div>
          </div>
          ...
          {/* Divider */}
          <hr className="border-[hsl(224,65%,95%)] dark:border-[hsl(224,65%,25%)]" />
          ...
          <div className="px-6 md:px-10 lg:px-12 py-8 md:py-10">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-8">
              ...
              <ul className="space-y-3 text-left">
                <li className="flex items-center text-xs md:text-sm text-[hsl(225,20%,60%)] dark:text-[hsl(225,20%,80%)]">
                  <Image
              ...
              {/* CTA Button */}
              <button 
                className="w-[80%] md:w-auto px-8 md:px-12 py-4 md:py-3 rounded-full text-sm font-extrabold hover:opacity-90 transition-opacity duration-200 whitespace-nowrap bg-[hsl(227,35%,25%)] dark:bg-[hsl(174,86%,45%)] text-[hsl(226,100%,87%)] dark:text-[hsl(227,35%,25%)] mx-auto md:mx-0"
              >
                Start my trial
              </button>
            </div>
